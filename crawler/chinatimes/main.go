...
func get_detail(detail_url string) string {
	...
	c2.OnHTML("body", func(e *colly.HTMLElement) {
		...
		e.ForEach("div[class='article-body'] > p ", func(_ int, el *colly.HTMLElement) {
			if len(el.Text)>0 {
				// logrus.Info("*->", el.Text)
	...
	c2.Visit("https://www.chinatimes.com" + detail_url)
...
func Get_list(get_count int) [] mongodb.Data_field {
	...
	c.OnHTML("section[class='article-list'] > ul ", func(e *colly.HTMLElement) {
		// fmt.Println(e.Text)
		logrus.Info(e.ChildText(".title"))
		// fmt.Println(e.ChildText(".story"))
		e.ForEach("li", func(_ int, el *colly.HTMLElement) {
			...
			p_article := el.ChildText("div > div > div > h3")
			p_date_str := el.ChildAttr("div > div > div > div > time","datetime")
			p_tag := el.ChildText("div > div > div > div > div[class='category']")
			p_link := el.ChildAttr("div > div > div > h3 > a", "href")
			p_content := get_detail(p_link)
			p_date, err := time.Parse("2006-01-02 15:04:05", p_date_str+":00")
			// fmt.Println("****", p_tag, p_article, p_date , p_link, p_content)
			if err != nil {
			...
			fetch_data := mongodb.Data_field {
				Pid:		  		"chinatimes-"+hex.EncodeToString(pid_ecrpt[:]),
				Pub_date:  			p_date,
				Title:  			p_article,
				Page_link: 			"https://www.chinatimes.com"+p_link,
				Search_key: 		"*sys-即時新聞",
				Author: 	  		"-",
				Content:   			p_content,
				Score:				0,
				Cat_tag:   			p_tag,
				Channel_url: 		"chinatimes",
	...
	c.Visit("https://www.chinatimes.com/realtimenews/?chdtv")
